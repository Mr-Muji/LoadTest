...
import (
	...
	"github.com/Mr-Muji/LoadTest/logger" // 루트 디렉토리의 logger 패키지 추가
...
func main() {
	// 로거 초기화 호출
	tester.InitLogger("path/to/logfile.log") // 로그 파일 경로 설정
	...
	// /test 경로로 들어오는 HTTP 요청을 StartTestHandler 함수로 처리하도록 등록
	http.HandleFunc("/test", handler.StartTestHandler)
